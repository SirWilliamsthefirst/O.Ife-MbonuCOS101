...
fn login(){
    ...
    let mut login = LoginInfo{//intializing a variable to hold the user's Info; Their Username and Password.
...
impl LoginInfo{
    fn info_check(&mut self)->bool{
        ...
        let upper_case_present = self.password != self.password.to_lowercase();//to check if Uppercase letters are present.
        ...
        match errors {//match statement to print out the error of the password. 
...
fn file_creator(){//code to create files.
    ...
    for pointer in 0..company_names.len(){//If a company's liability is less than the target the it's leverage gets multipled by the targer percentage leverage multiplier, it's 5%.
